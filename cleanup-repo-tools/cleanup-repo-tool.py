...
# 判断是否执行删除操作
DELETE = "-d" in sys.argvwith open(LOG_PATH, "a") as log_file:
    ...
    for arch in ["aarch64", "any", "riscv64", "x86_64"]:
        ...
        for package_file in os.listdir(arch_dir):
            ...
            versions = sorted(versions, key=lambda x: (x[1], int(x[2]) if x[2].isdigit() else 0), reverse=True)
                    ...
                    if os.path.exists(os.path.join(arch_dir, delete_version)):
                        if DELETE:
                            ...
                            if os.path.exists(sig_file):
                                ...
                                log_file.write(f"Deleted: {delete_version} and added {delete_version}.sig to deletion list\n")
                            else:
                                log_file.write(f"Signature file not found for {delete_version}\n")
                        else:
                            log_file.write(f"To be deleted: {delete_version} and {delete_version}.sig\n")
            else:
